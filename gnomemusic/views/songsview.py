...
from gnomemusic.player import DiscoveryStatus, PlayerPlaylist
...
class SongsView(BaseView):
    ...
    def __init__(self, window, player):
        ...
        self.player.connect('song-discovered', self._on_song_discovered)
    ...
    def _on_item_activated(self, widget, id, path):
        ...
        if self.model[itr][8] != self._error_icon_name:
            self.player.set_playlist(PlayerPlaylist.Type.SONGS,
                                     None, self.model, itr)
            self.player.play()

    @log
    def update_model(self, player, current_song, position):
    ...
    @log
    def _on_song_discovered(self, player, index, status):
        if not player.playing_playlist(PlayerPlaylist.Type.SONGS, None):
            return
