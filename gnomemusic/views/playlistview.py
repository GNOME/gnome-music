...
class PlaylistView(BaseView):
    ...
    def _on_song_deleted(self, model, path):
        ...
        if new_pos > prev_pos:
            pos -= 1
        new_iter = model.get_iter_from_string(str(pos))
        current_song = model[new_iter][5]
        # current played song position has changed
        if self.player.get_current_media().get_id() == current_song.get_id():
            self._iter_to_clean = new_iter
            self._iter_to_clean_model = model
            self.player.set_playlist(
                'Playlist', self._current_playlist.get_id(), model, new_iter)
        else:
            song_path = self.player.currentTrack.get_path()
            new_iter = model.get_iter_from_string(song_path.to_string())
            self.player.set_playlist(
                'Playlist', self._current_playlist.get_id(), model,
                new_iter)
