...
# Idk, how to correctly name this class. Need help
class PauseOnSuspend(GObject.GObject):
    ...
    def __init__(self, player):
        ...
        self._fd = -1
        self._previous_status = Playback.STOPPED
    ...
    def _on_playback_status_changed(self, arguments):
        ...
        if self._previous_status != new_status:
            if new_status == Playback.PLAYING and self._fd == -1:
                self._take_lock()
    ...
    def _take_lock(self):
        self._suspend_proxy.call(
            "Inhibit",
            # Is codestyle of this declaraion OK?
            GLib.Variant(
                "(ssss)",
                (
                    "sleep",
    ...
    def _pause_playing(self, proxy, sender, signal, parameters):
        if (signal == "PrepareForSleep"
                and self._player.get_playback_status() == Playback.PLAYING):
            self._player.pause()
