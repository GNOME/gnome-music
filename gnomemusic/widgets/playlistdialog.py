...
class PlaylistDialog(Gtk.Dialog):
    ...
    _normal_state = Gtk.Template.Child()
    _empty_state = Gtk.Template.Child()
    ...
    def _set_view(self):
        if self._user_playlists_available:
            ...
            self._add_playlist_stack.props.visible_child = self._normal_state
            self._add_playlist_button = self._new_playlist_button
            self._add_playlist_entry = self._new_playlist_entry
        else:
            self._empty_state.show()
            self._add_playlist_stack.props.visible_child = self._empty_state
            self._add_playlist_button = self._first_playlist_button
            self._add_playlist_entry = self._first_playlist_entry

        self._add_playlist_button.connect('clicked', self._on_editing_done)

        self._add_playlist_entry.connect(
            'changed', self._on_add_playlist_entry_changed)
        self._add_playlist_entry.connect('activate', self._on_editing_done)
        self._add_playlist_entry.connect(
            'focus-in-event', self._on_add_playlist_entry_focused)
    ...
    def _on_editing_done(self, sender, data=None):
        if self._add_playlist_entry.props.text != '':
            self._playlist.create_playlist(self._add_playlist_entry.props.text)
