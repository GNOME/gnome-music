...
class LastFmScrobbler():
        ...
        try:
            ...
            for obj in accounts:
                ...
                if account.props.provider_name == "Last.fm":
                    self._authentification = obj.get_oauth2_based()
                    return
    ...
    @log
    def _scrobble_song(self, media, time_stamp):
        """Internal method called by self.scrobble_song"""
        self._playlists.update_all_static_playlists()

        if self._authentification is None:
            return

        api_key = self._authentification.props.client_id
        sk = self._authentification.call_get_access_token_sync(None)[0]
        secret = self._authentification.props.client_secret

        artist = utils.get_artist_name(media)
        title = utils.get_media_title(media)

        sig = ("api_key%sartist[0]%smethodtrack.scrobblesk%stimestamp[0]"
               "%strack[0]%s%s") % \
        ...
        try:
            ...
            if r.status_code != 200:
                logger.warn("Failed to scrobble track: %s %s" %
    ...
    @log
    def scrobble_song(self, media, time_stamp):
        ...
        t = Thread(target=self._scrobble_song, args=(media, time_stamp))
        t.setDaemon(True)
        t.start()

    @log
    def _update_now_playing(self, media):
        """Internal method called by self.update_now_playing"""
        api_key = self._authentification.props.client_id
        sk = self._authentification.call_get_access_token_sync(None)[0]
        secret = self._authentification.props.client_secret
        ...
        sig = "api_key%sartist%smethodtrack.updateNowPlayingsk%strack%s%s" % \
              (api_key, artist, sk, title, secret)
    ...
    @log
    def update_now_playing(self, media):
        ...
        t = Thread(target=self._update_now_playing, args=(media,))
        t.setDaemon(True)
        t.start()
