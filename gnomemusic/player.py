...
from gi.repository import Gtk, GLib, Gio, GObject, Grl, Gst, GstPbutils
...
class Player(GObject.GObject):
    ...
    __gsignals__ = {
        ...
        'current-changed': (GObject.SignalFlags.RUN_FIRST, None, (Grl.Media,)),
    ...
    def _load(self, media):
        ...
        if self.current_track and self.current_track.valid():
            ...
            self.emit('current-changed', media)
    ...
    @log
    def set_playback_status(self, state):
        # FIXME: Just a proxy right now.
        self._player.state = state
