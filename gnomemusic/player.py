...
class PlayerPlaylist(GObject.GObject):
    ...
    def set_playlist(self, playlist_type, playlist_id, model, model_iter):
        ...
        # Playlist is the same, nothing else to change.
        ...
        GLib.idle_add(self._validate_all_songs)
    ...
    def change_position(self, prev_pos, new_pos):
        ...
        current_song_id = current_song[PlayerField.SONG].get_id()
        ...
        if (self._current_index >= first_pos
            ...
            for index, song in enumerate(self._songs[first_pos:last_pos + 1]):
                if song[PlayerField.SONG].get_id() == current_song_id:
    ...
    def add_song(self, song, song_index):
        ...
        self._validate_song(song_index, item)
    ...
    def _on_discovered(self, discoverer, info, error):
        index = self._validation_indexes[info.get_uri()][0]
        field = PlayerField.VALIDATION
        del(self._validation_indexes[info.get_uri()][0])
        if error:
    ...
    @log
    def _validate_song(self, index, item):
    ...
    @log
    def _validate_all_songs(self):
        for index, item in enumerate(self._songs):
            self._validate_song(index, item)
