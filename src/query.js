...
function album_songs (artistName, albumTitle) {
    var query = "SELECT rdf:type(?song) tracker:id(?song) as id nie:url(?song) as url  nie:title(?song) as title nmm:artistName(nmm:performer(?song)) as artist  nie:title(nmm:musicAlbum(?song)) as album nfo:duration(?song) as duration { ?song a nmm:MusicPiece ; nmm:performer [ nmm:artistName '%an' ]  ; nmm:musicAlbum  [ nmm:albumTitle '%at' ] } ORDER BY tracker:added(?song)";
